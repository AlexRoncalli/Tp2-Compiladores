...
Token* palavra(){
    ...
    while(isalpha(linha_do_arquivo[i]) || isdigit(linha_do_arquivo[i]) || linha_do_arquivo[i] == '_'){
    	if(j > MAX-10){
...
Token* constante_numerica(){
    ...
    while(isdigit(linha_do_arquivo[i]) || linha_do_arquivo[i] == '.'){
    	if(isdigit(linha_do_arquivo[i])){
    		...
    		i++, k++, erro = 0;
    	}else if(ponto_flutuante == 0 && linha_do_arquivo[i] == '.'){
    		constante[k] = linha_do_arquivo[i];
